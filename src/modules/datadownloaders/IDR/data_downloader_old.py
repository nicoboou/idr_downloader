...
class DataDownloader:
    ...
    def download(
        ...
        # Instanciate dataset_dict
        dataset_dict = {}

        # Instanciate images_resolution_dict: it is painful to get the resolution of the images from IDR at ANY tim, so we will store the resolution of the images in a dict to better handle it later
        images_resolution_dict = {}

        # Get image ids
        image_ids = self.retrieve_image_ids(metadata_file=self.metadata_filepath)  # images_ids is a dict
        ...
        )  # random_image_ids_selected is a dict
        ...
        for experiment_id in tqdm(random_image_ids_selected.keys(), file=sys.stdout, desc="Experiments"):
            if bool(random_image_ids_selected[experiment_id]):
                dataset_dict[experiment_id] = {}
                images_resolution_dict[experiment_id] = []
                image_ids_list = random_image_ids_selected[experiment_id]
                number_of_images_not_downloaded = 0

                for image_id in tqdm(image_ids_list, file=sys.stdout, desc="Images"):
                    # Download image
                    try:
                        (
                            image_numpy,
                        ...
                        print(self.save_directly)
                        print(self.file_format)
                        if self.save_directly and self.file_format == "png":
                        ...
                        # Save to dict
                            ...
                            # Append image numpy array to the dict
                            dataset_dict[experiment_id][image_id] = image_numpy

                            # Append image resolution to the dict
                            images_resolution_dict[experiment_id].append(image_numpy.shape)

                    except Exception as e:
                        print("*" * 100)
                        print(f"Error downloading image {image_id} from experiment {experiment_id}: {e}")
                        print("*" * 100)
                        number_of_images_not_downloaded += 1
                        # Continue to the next iteration of the loop
                        continue

                total_imgs_downloaded = num_images_requested - number_of_images_not_downloaded
                print(
                    f"\nImages downloaded for {experiment_id}. \nTOTAL NUMBER OF IMAGES DOWNLOADED: {total_imgs_downloaded}/{num_images_requested}"
                )

                # Store in download_tracker.csv
                df_path = "data/IDR/cell/download_tracker.csv"
                print(f"Storing download info in {df_path}")

                df_download_tracker = pd.read_csv(df_path)
                row_to_append = {
                    "metadata_filepath": self.metadata_filepath,
                    "experiment_id": experiment_id,
                    "#images_requested": num_images_requested,
                    "#images_downloaded": total_imgs_downloaded,
                    "#images_missing": number_of_images_not_downloaded,
                    "#percentage_downloaded": (total_imgs_downloaded / num_images_requested * 100),
                    "#percentage_missing": (number_of_images_not_downloaded / num_images_requested * 100),
                }

                df_download_tracker.loc[len(df_download_tracker)] = row_to_append
                df_download_tracker.to_csv(df_path, index=False)

            else:
                dataset_dict[experiment_id] = {}

        return dataset_dict, random_image_ids_selected, images_resolution_dict
    ...
    def download_image(
        ...
        try:
            ...
            random_time_sleep = random.uniform(1.2, 3.7)
...
def main(args):
    ...
    # Download images
    print(f"args.save_directly:{args.save_directly}")

    data_downloader = DataDownloader(
        metadata_filepath=args.metadata_filepath,
        file_format=args.format,
        save_path=args.save_path,
        save_directly=args.save_directly,
    )
    dataset_dict, random_image_ids_selected, images_resolution_dict = data_downloader.download(
    ...
    if not args.save_directly:
        print(f"Saving images (.{args.format} format) at {args.save_path}...")
        ...
        for experiment_id, experiment_images in dataset_dict.items():
            ...
            if bool(experiment_images):
                ...
                experiment_images_list, experiment_image_ids = (
                    list(experiment_images.values()),
                    list(experiment_images.keys()),
                )

                # Save images
                if args.format == "png":
                    filename = args.save_path + str(experiment_id)
                    DataDownloader.save_to_png(
                    ...
                    filename = args.save_path + str(experiment_id) + ".npz"
